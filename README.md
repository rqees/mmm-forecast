...
Then: upload a weekly CSV, confirm the column mapping, train, forecast, backtest, save. To try it without your own data, upload `data/sample_weekly.csv`, a synthetic file in the same schema (118 weeks, six channels, a query-volume control). The full walkthrough is in [docs/DOCUMENTATION.md](docs/DOCUMENTATION.md#4-getting-started).
...
data/sample_weekly.csv  Synthetic sample in the expected schema; data/make_sample.py regenerates it
notebooks/              Meridian's sample walkthrough and the v1 notebook pipeline the app grew out of
```

Built during a four-week data science internship at Jellyfish, August to September 2026. Client data is not included; the sample dataset is generated by `data/make_sample.py` and no row in it comes from the client file. The figures above are from the client engagement.
